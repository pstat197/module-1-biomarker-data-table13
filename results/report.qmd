...
![](images/Q1Comparison copy.png){fig-align="center"}
...
![](images/q2top13.png){alt="Question 1 comparison" fig-align="center" width="115"}
...
![](images/q2boxplot.png){alt="Question 1 comparison" fig-align="center" width="450"}
...
![](images/q2proptest.png){alt="Question 1 comparison" fig-align="center" width="150"}
...
![](images/q3_plot_1 copy.png){width="50%"}

![](images/q3_plot_2 copy.png){width="50%"}
...
Compared to the $\underline{benchmark}$ results: 
$$
\begin{array}{llr}
\hline
\textbf{metric} & \textbf{estimator} & \textbf{estimate} \\
...
\end{array}
$$
Thus, the Guided LASSO approach made roughly an $8\%$ decrease in sensitivity, $27\%$ increase in specificity, $8\%$ increase in accuracy, and $3\%$ decrease in AUC. This method found a panel of 35 proteins which resulted in better ability of identifying true negatives and greater ability to recognize both true positives and true negatives out of all test results. The proteins selected for the classifier were:
...
Before attempting the Guided LASSO approach we implemented a pure LASSO model (giving the same L1 penalty to all proteins) with the given metrics:
...
comparison_data <- data.frame(
  ...
  KeyInsight = c(
    ...
    "Reduced ability to correctly identify ASD cases.",
    "Unchanged performance in ruling out controls.",
    "Lower overall discrimination ability."
  )
)
...
knitr::kable(
  comparison_data,
  caption = "Comparative Performance and Insights: Pure LASSO vs. In-class Baseline",
  col.names = c(
    "Metric", 
  ...
  align = 'lcccc',  # Left-align first column, Center-align the rest
  digits = 3         # Ensure numerical columns are formatted nicely
)
```

![Question 4 Pure LASSO](images/Q4PureLasso copy.png){width="50%"}

While the performance metrics for this model were similar to the baseline across all metrics, the overall strategy of combining multiple feature selection methods (t-tests, Random Forest, and Guided LASSO) provided a robust and statistically validated pathway to stable panel selection.
...
The **Pure LASSO** model (43 proteins) sacrificed a small amount of overall discriminative ability (AUROC decreased by $\approx 2.8\%$) and Accuracy (decreased by $1\%$). The **Guided LASSO** approach provided a robust, multi-method validation pathway, resulting in a stable 35-protein panel with improved accuracy ($8\%$) and specificity ($27\%$) compared to the baseline.
...
3.  Biological Interpretation: Conduct a functional analysis of the 35 proteins selected by the Guided LASSO model to determine if they align with known biological pathways associated with ASD, thereby providing biological grounding for the statistical model.
